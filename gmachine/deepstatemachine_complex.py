...
from gmachine.complexoperation import add_node, add_one_node_rand_edge, add_n_nodes_n_edges, add_triangle_n_edges
...
class GraphEmbed(nn.Module):
    def __init__(self, node_hidden_size):
        ...
        self._conv = GraphConv(30, self.graph_hidden_size, allow_zero_in_degree=True)
...
class DarmGC(nn.Module):
    ...
    def __init__(self, v_max, node_hidden_size, num_prop_rounds):
        ...
        action_values = [
            ...
            DarmGCActions.add_one_node_rand_edge,
            DarmGCActions.add_triangle_n_edges,
            DarmGCActions.add_n_nodes_n_edges,
            DarmGCActions.stop
        ]
        ...
        self._state_adding_action = DeepStateModule(self._graph_embed.graph_hidden_size, action_values)
    ...
    def forward_train(self, actions):
        ...
        current_state = self._state_adding_action(cur_graph_embedding, self.current_action(actions))
        ...
        while not DarmGCActions.stop == int(current_state):
            if last_added_node_idx < 0 or DarmGCActions.add_node == int(current_state):
                # equivalent to add_node operation in construction sequence
                self._g.add_nodes(1)
                last_added_node_idx = self._g.number_of_nodes() - 1
                self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)
                current_state = self._state_adding_action(cur_graph_embedding,
                                                          self.get_action_and_increment_step(actions))

            elif DarmGCActions.add_one_node_rand_edge == int(current_state):
                # last_added_node_embedding = self._g.nodes[last_added_node_idx].data[GraphFeat.vertex_repr]

                # Equivalent to add_n_nodes_n_edges operation in construction sequence
                # add a node
                # get a random node from the existing dgl graph
                random_node = int(random.choice(self._g.nodes()))
                self._g.add_nodes(1)
                last_added_node_idx = self._g.number_of_nodes() - 1
                ...
                self._g.add_edges(last_added_node_idx, random_node)
                self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)
                self._initialize_edge_repr(last_added_node_idx, random_node)
            ...
            elif DarmGCActions.add_n_nodes_n_edges == int(current_state):
                # n nodes and n edges
                node_num = 3
                edge_num = 3
                ...
                node_list = [n for n in self._g.nodes()]

                for n in range(node_num):
                    self._g.add_nodes(1)
                    last_added_node_idx = self._g.number_of_nodes() - 1
                    self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)
                    graph_rand_node = int(random.choice(node_list))
                    self._g.add_edges(last_added_node_idx, graph_rand_node)
                    self._initialize_edge_repr(last_added_node_idx, graph_rand_node)
            ...
            elif DarmGCActions.add_triangle_n_edges == int(current_state):
                # last_added_node_embedding = self._g.nodes[last_added_node_idx].data[GraphFeat.vertex_repr]

                # Equivalent to add_triangle_n_edges operation in construction sequence
                edge_num = 3
                node_list = [n for n in self._g.nodes()]

                # Add three nodes of a triangle
                self._g.add_nodes(1)
                n1 = last_added_node_idx = self._g.number_of_nodes() - 1
                self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)
                self._g.add_nodes(1)
                n2 = last_added_node_idx = self._g.number_of_nodes() - 1
                self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)
                self._g.add_nodes(1)
                n3 = last_added_node_idx = self._g.number_of_nodes() - 1
                self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)

                # Add edges between the nodes to create a triangle
                self._g.add_edges(n1, n2)
                self._initialize_edge_repr(n1, n2)
                self._g.add_edges(n2, n3)
                self._initialize_edge_repr(n2, n3)
                self._g.add_edges(n1, n3)
                self._initialize_edge_repr(n1, n3)

                triangle_nodes = [n1, n2, n3]
                for i in range(edge_num):
                    tri_rand_node = int(random.choice(node_list))
                    graph_rand_node = random.choice(triangle_nodes)
                    if not self._g.has_edges_between(tri_rand_node, graph_rand_node):
                        self._g.add_edges(tri_rand_node, graph_rand_node)
                        self._initialize_edge_repr(tri_rand_node, graph_rand_node)
                ...
                # Add edge to the graph
                # self._g.add_edge(last_added_node_idx, random_node)
                # self._initialize_node_repr(last_added_node_idx, cur_graph_embedding)
                # self._initialize_edge_repr(last_added_node_idx, random_node)
    ...
    def forward_inference(self):
        ...
        while not DarmGCActions.stop == current_state and self._g.number_of_nodes() < self.v_max + 1:
            ...
            if last_added_node_idx < 0 or DarmGCActions.add_node == current_state:
                ...
                # current_graph_embedding_state = torch.cat([cur_graph_embedding, last_added_node_embedding], dim=1)
                current_state = self._state_adding_action(cur_graph_embedding)
                construction_sequence.append(DarmGCActions.add_one_node_rand_edge)
                ...
                # current_graph_embedding_state = torch.cat([cur_graph_embedding, last_added_node_embedding], dim=1)
                current_state = self._state_adding_action(cur_graph_embedding)
                construction_sequence.append(DarmGCActions.add_n_nodes_n_edges)
                ...
                # current_graph_embedding_state = torch.cat([cur_graph_embedding, last_added_node_embedding], dim=1)
                current_state = self._state_adding_action(cur_graph_embedding)
                construction_sequence.append(DarmGCActions.add_triangle_n_edges)
    ...
    def prepare_for_training(self):
        ...
        self._state_adding_action.prepare_for_training()

    def get_log_loss(self):
        loss = 0
        loss = self._state_adding_action.get_log_losses
        return loss
