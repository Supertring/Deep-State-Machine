...
#         self.graph_hidden_size = 2 * node_hidden_size
#         self.register_buffer('_embedding_unknown_graph', torch.zeros(1, self.graph_hidden_size))
#         self.conv1 = GraphConv(node_hidden_size, 64, allow_zero_in_degree=True)
#         self.conv2 = GraphConv(64, self.graph_hidden_size, allow_zero_in_degree=True)
#
#     def forward(self, g):
#         if g.number_of_nodes() == 0:
#             return self._embedding_unknown_graph
#         else:
#             hvs = get_node_features(g)
#             h = torch.nn.functional.normalize(hvs)
#
#             h = self.conv1(g, h)
...
#             h = torch.nn.functional.normalize(h)
#
#             hg = self.conv2(g, h).sum(0, keepdims=True)
#             return hg
